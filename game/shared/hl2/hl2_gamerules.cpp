...
#ifdef TACTICALTHOTS
ConVar	sk_max_sunnyd("sk_max_sunnyd", "3", FCVAR_REPLICATED);
ConVar	sk_max_yeet("sk_max_yeet", "1", FCVAR_REPLICATED);
ConVar	sk_plr_sunnyd_heal("sk_plr_sunnyd_heal", "25", FCVAR_REPLICATED);
ConVar	sk_plr_sunnyd_burptime("sk_plr_sunnyd_burptime", "30", FCVAR_REPLICATED);
ConVar	sk_plr_sunnyd_burptimelimit("sk_plr_sunnyd_burptimelimit", "30", FCVAR_REPLICATED);
ConVar	sk_plr_sunnyd_burptimeclamp("sk_plr_sunnyd_burptimeclamp", "120", FCVAR_REPLICATED);
#endif
...
ConVar  alyx_darkness_force( "alyx_darkness_force", "0", FCVAR_CHEAT | FCVAR_REPLICATED );
	...
	if( (info.GetDamageType() & DMG_CRUSH) && info.GetInflictor() && pVictim->MyNPCPointer() )
	{
		if( pVictim->MyNPCPointer()->IsPlayerAlly() )
		{
			...
			//Don't deal YEET damage to Ally NPCs.
#ifdef TACTICALTHOTS
			if (!Q_stricmp(info.GetAttacker()->GetClassname(), "npc_grenade_yeet"))
			{
				return false;
			}
#endif
...
CAmmoDef *GetAmmoDef()
{
	...
	if ( !bInitted )
	{
		...
		def.AddAmmoType("USP",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	15,							10,							100,					BULLET_IMPULSE(200, 1225), 0);
...
#ifdef TACTICALTHOTS
		def.AddAmmoType("Uranium",			DMG_BURN,					TRACER_NONE,			15,							15,							100,					0, 0);
		def.AddAmmoType("FlashGrenade",		DMG_BURN,					TRACER_NONE,			"sk_plr_dmg_grenade",		"sk_npc_dmg_grenade",		"sk_max_grenade",		0, 0);
		def.AddAmmoType("Sunnyd",			DMG_BULLET,					TRACER_NONE,			NULL,						NULL,						"sk_max_sunnyd",		0, 0);
		def.AddAmmoType("Yeet",				DMG_BULLET,					TRACER_NONE,			NULL,						NULL,						"sk_max_yeet",			0, 0);
#endif
