...
#ifdef TACTICALTHOTS
	virtual bool			ShouldDrawCrosshair( void ) { return !IsIronsighted() && !( GetWeaponType() == WEAPONTYPE_SNIPER_RIFLE && ( ToBasePlayer( GetOwner() ) && ToBasePlayer( GetOwner() )->GetFOV() != ToBasePlayer( GetOwner() )->GetDefaultFOV() ) ); }
#else
...
public:
	...
	virtual float			GetWeaponRecoilSlideLimit(void);

	virtual Vector			GetWeaponRecoilViewKickVector(void);
	virtual Vector			GetWeaponRecoilViewSlideVector(void);

	// utility function
	void					DoRecoil(CBasePlayer *pPlayer, Vector vecPunchAngles, float fireDurationTime, float slideLimitTime, Vector vecViewSlide);

	virtual WeaponSound_t	GetAttackSound(void);

	CWeaponType				GetWeaponType(void);
#endif
...
public:
	...
	bool					m_bHasSupressorOn;
	//For our dear USP
	bool					m_bYeetedEmpty = false;
