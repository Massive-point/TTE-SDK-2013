...
bool CBaseViewModel::ShouldReceiveProjectedTextures(int flags)
{
	if (cl_disable_vmodelptexture.GetBool())
	{
		return false;
	}
	else
...
void CBaseViewModel::CalcViewModelView(CBasePlayer *owner, const Vector& eyePosition, const QAngle& eyeAngles)
{
	...
	if ( pWeapon != NULL )
	{
...
#endif
		{
			...
			{
				pWeapon->AddViewmodelBob( this, vmorigin / 100, vmangles );
			}
			else
#endif
			{
...
void CBaseViewModel::CalcIronsights(Vector &pos, QAngle &ang)
{
	...
	if (delta < 0)
	{
		pWeapon->m_flIronsightedTime = gpGlobals->curtime - 1.0f;
		delta = (gpGlobals->curtime - pWeapon->m_flIronsightedTime) * 7.0f;

		//This shouldn't happen, but I leave that just in case.
		if (delta < 0)
		{
			Warning("Warning!!! Delta below 0!!!\n");
