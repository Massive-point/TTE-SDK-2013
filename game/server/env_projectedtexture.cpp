...
void CEnvProjectedTexture::InputSetLightColor(inputdata_t &inputdata)
{
	...
	Vector colorchange;
	colorchange.x = GammaToLinear(clr.r * (1.0f / 255.0f)) * (clr.a * (1.0f / 255.0f));
	colorchange.y = GammaToLinear(clr.g * (1.0f / 255.0f)) * (clr.a * (1.0f / 255.0f));
	colorchange.z = GammaToLinear(clr.b * (1.0f / 255.0f)) * (clr.a * (1.0f / 255.0f));

	//m_cLightColor = inputdata.value.Color32();
	m_LinearFloatLightColor = colorchange;
}
