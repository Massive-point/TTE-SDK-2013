...
#ifdef TACTICALTHOTS
ConVar tte_ironsight_method( "tte_ironsight_method", "0", FCVAR_ARCHIVE | FCVAR_REPLICATED, "\n 0 = Toggle ironsight\n 1 = HOLD to ironsight\n 2 = if hit ironsight button - toggle ironsight, if holding ironsight button - hold ironsight" );
#endif
...
#ifdef TACTICALTHOTS
	m_bIronsightStateChanged = false;

	m_flCheckIronsightTime = gpGlobals->curtime;
#endif
}
...
//					Warning( "In train mode with no train!\n" );
	...
	{
#ifdef TACTICALTHOTS
		if (m_nButtons & IN_ZOOM || IsSprinting())
#else
...
void CHL2_Player::StartSprinting( void )
{
	...
	if( GetActiveWeapon() )
	{
		...
		GetActiveWeapon()->SetWeaponIdleTime( gpGlobals->curtime );	// reset weapon idle time in order to immediately start run animation
	}
#endif
}
...
void CHL2_Player::StopSprinting( void )
{
	...
	if( GetActiveWeapon() )
	{
		GetActiveWeapon()->SetWeaponIdleTime( gpGlobals->curtime );
	}
#endif
}
...
void CHL2_Player::SuitPower_Update( void )
{
	...
	else if( m_HL2Local.m_bitsActiveDevices )
	{
		...
		{
#ifndef TACTICALTHOTS // sprint is unlimited now
...
#ifdef TACTICALTHOTS
//-----------------------------------------------------------------------------
// Purpose: Vietnam toggle/hold/fusion for ironsight
//-----------------------------------------------------------------------------
void CHL2_Player::CheckIronsightMethod()
{
	if( GetActiveWeapon() )
	{
		// the rest is in CBaseCombatWeapon::EnableIronsights and CBaseCombatWeapon::DisableIronsights
		if( m_bIronsightStateChanged && ( ( m_afButtonReleased & IN_IRONSIGHT ) || !( m_nButtons & IN_IRONSIGHT ) ) )
		{
			m_bIronsightStateChanged = false;
		}
		
		if( ( m_nButtons & IN_IRONSIGHT ) && !GetActiveWeapon()->m_bInReload )
		{
			if( tte_ironsight_method.GetInt() != 1 && gpGlobals->curtime > m_flCheckIronsightTime && !m_bIronsightStateChanged ) // toggle ironsight, check time based on sequence duration
			{
				GetActiveWeapon()->ToggleIronsights();

				m_flCheckIronsightTime = gpGlobals->curtime + 0.5f;
			}
			else if( tte_ironsight_method.GetInt() == 1 && ( !IsSprinting() || IsSprinting() && ( m_afButtonPressed & IN_IRONSIGHT ) ) )	// hold ironsight
			{
				if( !GetActiveWeapon()->IsIronsighted() )
				{
					GetActiveWeapon()->EnableIronsights();
				}
			}
		}
		else if( tte_ironsight_method.GetInt() > 0 && GetActiveWeapon()->IsIronsighted() )
		{
			if( tte_ironsight_method.GetInt() == 1 )
			{
				GetActiveWeapon()->DisableIronsights();
			}
			else if( tte_ironsight_method.GetInt() == 2 && gpGlobals->curtime > m_flCheckIronsightTime && ( m_afButtonReleased & IN_IRONSIGHT ) )	// fusion mode
			{
				GetActiveWeapon()->DisableIronsights();
			}
		}
...
void CHL2_Player::ItemPostFrame()
{
	...
	// Around line 900, the same file.
	//// disallow shooting while sprinting
	//if( IsSprinting() && GetActiveWeapon() )
	//{
	//	GetActiveWeapon()->m_flNextPrimaryAttack = gpGlobals->curtime + 0.25f; 
	//	GetActiveWeapon()->m_flNextSecondaryAttack = gpGlobals->curtime + 0.25f; 
	//}
#endif
}
