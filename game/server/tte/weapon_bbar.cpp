...
IMPLEMENT_SERVERCLASS_ST(CWeaponBootyBlasterAR, DT_WeaponBootyBlasterAR)
END_SEND_TABLE()

LINK_ENTITY_TO_CLASS( weapon_bbar, CWeaponBootyBlasterAR );
PRECACHE_WEAPON_REGISTER( weapon_bbar );

BEGIN_DATADESC( CWeaponBootyBlasterAR )
#ifdef TACTICALTHOTS
	DEFINE_FIELD( m_flDelayBetweenBursts, FIELD_TIME),
	DEFINE_FIELD( m_bAllowBurstAttack, FIELD_BOOLEAN),
#endif
END_DATADESC()

acttable_t	CWeaponBootyBlasterAR::m_acttable[] = 
{
...
IMPLEMENT_ACTTABLE(CWeaponBootyBlasterAR);
...
//=========================================================
CWeaponBootyBlasterAR::CWeaponBootyBlasterAR( )
{
	m_fMinRange1		= 1;
	m_fMaxRange1		= 5000;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponBootyBlasterAR::Precache( void )
{
	BaseClass::Precache();
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWeaponBootyBlasterAR::Equip( CBaseCombatCharacter *pOwner )
{
...
// Output : int
//-----------------------------------------------------------------------------
int CWeaponBootyBlasterAR::WeaponRangeAttack1Condition( float flDot, float flDist )
{
#ifdef HL2_EPISODIC
...
// Output : int
//-----------------------------------------------------------------------------
int CWeaponBootyBlasterAR::WeaponRangeAttack2Condition( float flDot, float flDist )
{
	return COND_NONE;
}
...
// Input  : *pOperator - 
//-----------------------------------------------------------------------------
void CWeaponBootyBlasterAR::FireNPCPrimaryAttack( CBaseCombatCharacter *pOperator, bool bUseWeaponAngles )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponBootyBlasterAR::Operator_ForceNPCFire( CBaseCombatCharacter *pOperator, bool bSecondary )
{
...
//-----------------------------------------------------------------------------
void CWeaponBootyBlasterAR::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
	switch( pEvent->event )
	{
...
//-----------------------------------------------------------------------------
float CWeaponBootyBlasterAR::GetMinRestTime( void )
{
...
//-----------------------------------------------------------------------------
float CWeaponBootyBlasterAR::GetMaxRestTime( void )
{
	if ( IsAlyxInInjuredMode() )
		return 3.0f;
...
#ifdef TACTICALTHOTS
void CWeaponBootyBlasterAR::PrimaryAttack()
{
	if (!m_bAllowBurstAttack)
		return;

	if( m_flDelayBetweenBursts <= gpGlobals->curtime ) 
	{
...
//		DispatchParticleEffect( "weapon_muzzle_smoke", PATTACH_POINT_FOLLOW, pOwner->GetViewModel(), "muzzle", true);
		...
		m_flDelayBetweenBursts = gpGlobals->curtime + 0.6f;
		m_bAllowBurstAttack = false;
	}
}

void CWeaponBootyBlasterAR::ItemPostFrame(void)
{
	...
	{
		m_bAllowBurstAttack = true;
	}
}

//-----------------------------------------------------------------------------
// no secondary attack for this weapon
//-----------------------------------------------------------------------------
void CWeaponBootyBlasterAR::SecondaryAttack()
{
	return;
}

bool CWeaponBootyBlasterAR::Reload(void)
{
	bool fRet;
...
void CWeaponBootyBlasterAR::AddViewKick( void )
{
