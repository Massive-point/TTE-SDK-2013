...
//
// Purpose: Just like item_item_crate, but has a custom model.
//
//=============================================================================//
...
//-----------------------------------------------------------------------------
class CItem_CrackCrate : public CPhysicsProp
{
public:
	DECLARE_CLASS(CItem_CrackCrate, CPhysicsProp);
	DECLARE_DATADESC();
...
LINK_ENTITY_TO_CLASS(item_crackcrate, CItem_CrackCrate);


//-----------------------------------------------------------------------------
// Save/load: 
//-----------------------------------------------------------------------------
BEGIN_DATADESC(CItem_CrackCrate)
...
// Purpose: 
//-----------------------------------------------------------------------------
void CItem_CrackCrate::Precache(void)
{
	// Set this here to quiet base prop warnings
	if (!GetModelName())
	{
		SetModelName(MAKE_STRING("models/items/item_item_crate.mdl"));
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CItem_CrackCrate::Spawn(void)
{
...
// Input  : &data - 
//-----------------------------------------------------------------------------
void CItem_CrackCrate::InputKill(inputdata_t &data)
{
	UTIL_Remove(this);
}
...
//-----------------------------------------------------------------------------
int CItem_CrackCrate::OnTakeDamage(const CTakeDamageInfo &info)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CItem_CrackCrate::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CItem_CrackCrate::OnBreak(const Vector &vecVelocity, const AngularImpulse &angImpulse, CBaseEntity *pBreaker)
{
...
void CItem_CrackCrate::OnPhysGunPickup(CBasePlayer *pPhysGunUser, PhysGunPickup_t reason)
{
