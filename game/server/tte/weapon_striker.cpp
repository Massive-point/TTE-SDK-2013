...
// Input  : *pOperator - 
//-----------------------------------------------------------------------------
void CWeaponStriker::FireNPCPrimaryAttack( CBaseCombatCharacter *pOperator, bool bUseWeaponAngles )
{
	...
	pOperator->FireBullets( 8, vecShootOrigin, vecShootDir, GetBulletSpread( (WeaponProficiency_t)0 ), MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 0 );
}
...
//-----------------------------------------------------------------------------
float CWeaponStriker::GetMinRestTime()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CWeaponStriker::GetMaxRestTime()
{
	if( hl2_episodic.GetBool() && GetOwner() && GetOwner()->Classify() == CLASS_COMBINE )
	{
		return 1.5f;
...
//-----------------------------------------------------------------------------
float CWeaponStriker::GetFireRate()
{
	if( hl2_episodic.GetBool() && GetOwner() && GetOwner()->Classify() == CLASS_COMBINE )
	{
		return 0.8f;
...
// Output :
//-----------------------------------------------------------------------------
//bool CWeaponStriker::StartReload( void )
//{
...
// Output :
//-----------------------------------------------------------------------------
//void CWeaponStriker::FinishReload( void )
//{
...
// Output :
//-----------------------------------------------------------------------------
//void CWeaponStriker::FillClip( void )
//{
//	CBaseCombatCharacter *pOwner  = GetOwner();
//	
//	if ( pOwner == NULL )
...
void CWeaponStriker::PrimaryAttack( void )
{
	...
	pPlayer->FireBullets( sk_plr_num_shotgun_pellets.GetInt(), vecSrc, vecAiming, GetBulletSpread( (WeaponProficiency_t)0 ), MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 0, -1, -1, 0, NULL, true, true );
...
//-----------------------------------------------------------------------------
// Purpose: No secondary attack for this shotgun
//-----------------------------------------------------------------------------
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponStriker::ItemHolsterFrame( void )
{
	// Must be player held
...
//==================================================
/*
void CWeaponStriker::WeaponIdle( void )
{
	//Only the player fires this way so we can cast
