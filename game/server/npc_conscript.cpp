...
Class_T	CNPC_Conscript::Classify ( void )
{
	...
	{
		return	CLASS_COMBINE; //Only hostile temporarily, will change to CLASS_CONSCRIPT
...
// dead enemy
			...
			if (HasCondition(COND_NEW_ENEMY)) //Comment out or delete the Msgs here unless you like your dev console getting spammed -Stacker
			{
				...
				if (iPercent <= 25 && GetEnemy() != NULL) 
					{
						DevMsg("CONSCRIPT (balanced): CHOSE FALLBACK!\n");
						return SCHED_RUN_FROM_ENEMY_FALLBACK;
					}
					else if (iPercent <= 50 && GetEnemy() != NULL) 
					{
						DevMsg("CONSCRIPT (balanced): CHOSE TO TAKE COVER!\n");
						return SCHED_TAKE_COVER_FROM_ENEMY;
					}
					else if (iPercent >= 75 && GetEnemy() != NULL) 
					{
						DevMsg("CONSCRIPT (balanced): CHOSE ESTABLISH LOF!\n");
				...
				if (iPercent <= 30 && GetEnemy() != NULL)
					{
						DevMsg("CONSCRIPT (cautious): CHOSE TO TAKE COVER!\n");
						return SCHED_TAKE_COVER_FROM_ENEMY;
					}
					else if (iPercent <= 75 && GetEnemy() != NULL) 
					{
						DevMsg("CONSCRIPT (cautious): CHOSE CAUTIOUS TAKECOVER!\n");
						return SCHED_CONSCRIPT_CAUTIOUS_TAKECOVER;
					}
					else if (iPercent >= 25 && GetEnemy() != NULL) 
					{
						DevMsg("CONSCRIPT (cautious): CHOSE ESTABLISH LOF!\n");
						return SCHED_ESTABLISH_LINE_OF_FIRE;
					}
				}
				...
				if (iPercent <= 50 && GetEnemy() != NULL)
					{
						DevMsg("CONSCRIPT (aggressive): CHOSE CHASE ENEMY!\n");
						return SCHED_CHASE_ENEMY;
					}
					else if (iPercent <= 15 && GetEnemy() != NULL) 
					{
						DevMsg("CONSCRIPT (aggressive): CHOSE TO TAKE COVER!\n");
						return SCHED_TAKE_COVER_FROM_ENEMY;
					}
					else if (iPercent >= 75 && GetEnemy() != NULL) 
					{
						DevMsg("CONSCRIPT (aggressive): CHOSE ESTABLISH LOF!\n");
						return SCHED_ESTABLISH_LINE_OF_FIRE;
					}
				}
			...
			if (HasCondition(COND_ENEMY_OCCLUDED))
			{
				...
				if ( m_iPersonality == CONSCRIPT_PERSONALITY_BALANCED ) //Voicelines here are commented out due to not being implemented yet -Stacker
				{
					...
					{
						return SCHED_ESTABLISH_LINE_OF_FIRE;
						DevMsg("CONSCRIPT: I'M PICKING ESTABLISH LOF AGAINST OCCLUDED ENEMY!\n");
						//Speak ( CONSCRIPT_GOINGAFTER);
					}
					else if (iPercent <= 60 && GetEnemy() != NULL)
					{
						return SCHED_CONSCRIPT_SUPPRESSINGFIRE;
						DevMsg("CONSCRIPT: I'M PICKING SUPPRESSING FIRE AGAINST OCCLUDED ENEMY!\n");
						//Speak (CONSCRIPT_SUPPRESS);
					}
					else if (iPercent <= 40 && GetEnemy() != NULL)
					{
						return SCHED_STANDOFF;
						DevMsg("CONSCRIPT: I'M PICKING STANDOFF AGAINST OCCLUDED ENEMY!\n");
				...
				if (m_iPersonality == CONSCRIPT_PERSONALITY_CAUTIOUS) //more likely to use suppressing fire/standoff
				{
					...
					{
						return SCHED_ESTABLISH_LINE_OF_FIRE;
						DevMsg("CONSCRIPT (cautious): I'M PICKING ESTABLISH LOF AGAINST OCCLUDED ENEMY!\n");
						//Speak ( CONSCRIPT_GOINGAFTER);
					}
					else if (iPercent <= 60 && GetEnemy() != NULL)
					{
						return SCHED_CONSCRIPT_SUPPRESSINGFIRE;
						DevMsg("CONSCRIPT (cautious): I'M PICKING SUPPRESSING FIRE AGAINST OCCLUDED ENEMY!\n");
						//Speak (CONSCRIPT_SUPPRESS);
					}
					else if (iPercent <= 30 && GetEnemy() != NULL)
					{
						return SCHED_STANDOFF;
						DevMsg("CONSCRIPT (cautious): I'M PICKING STANDOFF AGAINST OCCLUDED ENEMY!\n");
						//Speak (CONSCRIPT_TAUNT);
					}
				}
				...
				if ( m_iPersonality == CONSCRIPT_PERSONALITY_AGGRESSIVE )//more likely to go after occluded enemies
				{
					...
					{
						return SCHED_ESTABLISH_LINE_OF_FIRE;
						DevMsg("CONSCRIPT (aggressive): I'M PICKING ESTABLISH LOF AGAINST OCCLUDED ENEMY!\n");
						//Speak ( CONSCRIPT_GOINGAFTER);
					}
					else if (iPercent <= 20 && GetEnemy() != NULL)
					{
						return SCHED_CONSCRIPT_SUPPRESSINGFIRE;
						DevMsg("CONSCRIPT (aggressive): I'M PICKING SUPPRESSING FIRE AGAINST OCCLUDED ENEMY!\n");
						//Speak (CONSCRIPT_SUPPRESS);
					}
					else if (iPercent <= 30 && GetEnemy() != NULL)
					{
						return SCHED_CHASE_ENEMY;
						DevMsg("CONSCRIPT (aggressive): I'M GOING AFTER OCCLUDED ENEMY!\n");
						//Speak (CONSCRIPT_TAUNT);
					}
				}
...
WeaponProficiency_t CNPC_Conscript::CalcWeaponProficiency( CBaseCombatWeapon *pWeapon )	// HEY TTE DEVS, YOU WILL WANT TO ADJUST THIS FOR ANY NEEW WEAPONS YOU WANT CONSCRIPTS TO WIELD
{
...
//=========================================================
// > SCHED_CONSCRIPT_BARNACLE_CHEW since when is a conscript gonna chew on things?
//=========================================================
AI_DEFINE_SCHEDULE
(
