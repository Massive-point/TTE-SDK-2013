...
void CHudScope::Init( void )
{
	...
	vgui::surface()->DrawSetTextureFile(m_iScopeArcTexture, "overlays/scope_arc", true, false);
...
{
	C_BasePlayer *pPlayer = dynamic_cast<C_BasePlayer *>(C_BasePlayer::GetLocalPlayer());
	...
	if( pPlayer->GetFOV() != pPlayer->GetDefaultFOV() && pWeapon->GetWeaponType() == WEAPONTYPE_SNIPER_RIFLE )
	{
		int screenWide, screenTall;
		GetHudSize(screenWide, screenTall);
