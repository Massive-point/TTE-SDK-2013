...
#ifdef TACTICALTHOTS
	if (player->GetActiveWeapon() && (player->GetActiveWeapon()->IsIronsighted()
		|| (player->GetActiveWeapon()->GetWeaponType() == WEAPONTYPE_SNIPER_RIFLE && player->GetFOV() != player->GetDefaultFOV())))
	{
		return false;
	}
#endif
...
#ifdef TACTICALTHOTS
	CHL2_Player *pPlayer = dynamic_cast<CHL2_Player*>(player);

	if (pPlayer)
	{
		bFadeOut |= pPlayer->IsSprinting();

		if (pPlayer->GetActiveWeapon())
		{
			bFadeOut |= pPlayer->m_bHolsteredAW;
		}
	}
#endif
