...
// Purpose: 
//-----------------------------------------------------------------------------
void __MsgFunc_DoClientSideRecoil( bf_read &msg ) 
{
	...
	if( pPlayer )
	{
		...
		if( pWeapon )
		{
			Vector vecViewSlide = pWeapon->GetWeaponRecoilViewSlideVector();

			pPlayer->SetRecoilNewOffset( QAngle( vecViewSlide.x, vecViewSlide.y * RandomInt( -1, 1 ), vecViewSlide.z ) );
		}
	}
}
#endif
...
// -------------------------------------------------------------------------------- //
	...
	IMPLEMENT_CLIENTCLASS_DT(C_BasePlayer, DT_BasePlayer, CBasePlayer)
		...
		RecvPropInt		(RECVINFO(m_iBossHealthBarMax)),
		RecvPropInt		(RECVINFO(m_iBossHealthBarCur)),
		RecvPropInt		(RECVINFO(m_iBossHealthBarActive)),
		RecvPropString	(RECVINFO(m_iszBossNameHUD)),
#endif
...
#ifdef TACTICALTHOTS
		RecvPropFloat	(RECVINFO(m_flCalculatedViewAngleZ) ),
		RecvPropFloat	(RECVINFO(m_flCalculatedViewOffsetRight) ),
#endif
...
#ifdef TACTICALTHOTS
	DEFINE_FIELD( m_angRecoilOffset, FIELD_VECTOR ),
#endif
...
#ifdef TACTICALTHOTS
	usermessages->HookMessage( "NeedsClientSideRecoil", __MsgFunc_DoClientSideRecoil );

	m_angRecoilOffset.Init();

	// tte leaning
	m_flCalculatedViewOffsetRight = 0.0f;
	m_flCalculatedViewAngleZ = 0.0f;
	m_flMainOsscilate = 0.0f;
#endif
}
...
#ifdef TACTICALTHOTS
		// Make recoil more smooth
		//
		//Binka's message:
		//
		//It used to cause quite a serious bug when fired at angle with negative value. For example  pitch : -30 yaw : -30.
		//Basically m_angRecoilOffset was incremented instead of being decremented and it caused the game to go crazy.
		//I fixed the problem by UTIL_AngleDiff.

		QAngle currAnglesOld = pCmd->viewangles;
		QAngle currAngles = pCmd->viewangles;
		QAngle currDiff;
		QAngle angFinalAngles = currAngles - m_angRecoilOffset;
		QAngle zeroRCSet(0, 0, 0);

		float speed = 64.0f;

		if (m_angRecoilOffset.x < 1.0f)
		{
			speed = 4.0f;
		}
		else if (m_angRecoilOffset.x < 4.0f)
		{
			speed = 24.0f;
		}

		if ((currAngles - angFinalAngles).Length() > 0)
		{
			currAngles.x = ApproachAngle(angFinalAngles.x, currAngles.x, speed * gpGlobals->frametime);
			currAngles.y = ApproachAngle(angFinalAngles.y, currAngles.y, speed * gpGlobals->frametime);
			currAngles.z = ApproachAngle(angFinalAngles.z, currAngles.z, speed * gpGlobals->frametime);

			currDiff.x = UTIL_AngleDiff(currAnglesOld.x, currAngles.x);
			currDiff.y = UTIL_AngleDiff(currAnglesOld.y, currAngles.y);
			currDiff.z = UTIL_AngleDiff(currAnglesOld.z, currAngles.z);

			//Decrement the recoil.
			m_angRecoilOffset = m_angRecoilOffset - currDiff;

			//Warning(" tick_count: %i\n RecoilOffset: %5.2f %5.2f %5.2f \n currDiff: %5.2f %5.2f %5.2f \n",
			//	pCmd->tick_count,
			//	m_angRecoilOffset.x,
			//	m_angRecoilOffset.y,
			//	m_angRecoilOffset.z,
			...
			//Normalize Angles(Convert angles to -180 t 180 range).
			NormalizeAngles(currAngles);
			pCmd->viewangles = currAngles;

			//Clear the recoil Offset when it's zero.
			if ((zeroRCSet - m_angRecoilOffset).Length() <= 0.001)
			{
				//Warning("Clear recoil Offset\n");
				m_angRecoilOffset.x = 0;
				m_angRecoilOffset.y = 0;
				m_angRecoilOffset.z = 0;
			}
		}
#endif
	}
